...
import {addWordToFirebase, removeWordFromFirebase} from '../../firebase/firebase-helpers';
...
const Word = ({searchWord}) => {
    ...
    const {word, info, notFound, loading} = useWordAPI(searchWord);
    ...
    const handleRemoveFromDictionary = useCallback(() => {
        ...
        removeWordFromFirebase(user.email, word);
    });
    ...
    return loading ? (
        ...
        <>
            <section>
                <h1 className="text-gray-700 dark:text-white  font-bold font-serif text-4xl md:text-6xl my-8 ">
                    {word.toLowerCase()}
                </h1>
                {info.pronunciations &&
                    info.pronunciations.map(({audio, context, transcriptions}, index) => (
                        <div key={index}>
                            <div className="flex justify-between items-center font-sans text-lg md:text-3xl font-light">
                                ...
                                {audio && (
                                    <button
                                        ...
                                        className="bg-purple-100 dark:bg-purple-500 transition-all active:bg-purple-200 active:outline-none active:ring active:ring-purple-300  w-12 h-12 flex justify-center items-center rounded-full ">
                                        <svg
                                            viewBox="0 0 19 23"
                                            fill="none"
                                            ...
                                            className="w-4">
                                            <path
                                                className="fill-purple-500 dark:fill-purple-200"
                            ...
                            <hr className="w-full border-gray-100 dark:border-slate-700" />
                        </div>
                    ))}
                {user ? (
                    <button
                        onClick={
                            isInDictionary ? handleRemoveFromDictionary : handleAddToDictionary
                        }
                        className={`flex justify-center items-center w-full md:w-auto rounded-md font-large leading-7  my-4 px-3.5 py-3 ${
                            isInDictionary
                                ? 'bg-green-100 text-green-500 hover:bg-green-200 dark:bg-green-500 dark:text-white dark:hover:bg-green-600'
                                : 'bg-purple-100 text-purple-500 hover:bg-purple-200 dark:bg-purple-500 dark:text-white dark:hover:bg-purple-600'
                        }`}>
                        {isInDictionary ? (
                            <>
                                <svg
                                ...
                                <span>Added to your dictionary</span>
                            </>
                        ) : (
                            <>
                                <svg
                    ...
                    <button
                        onClick={logIn}
                        className={`flex justify-center items-center w-full md:w-auto rounded-md font-large leading-7  my-4 px-3.5 py-3 bg-purple-100 text-purple-500 hover:bg-purple-200 dark:bg-purple-500 dark:text-white dark:hover:bg-purple-600`}>
                        <svg
                            viewBox="0 0 182 182"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                    ...
                    info.lexemes.map(({partOfSpeech, senses, antonymSets, synonymSets}, index) => (
                        <div key={index}>
                            <h2 className="heading font-bold text-xl flex items-center">
                                ...
                                <hr className="w-full h-px border-gray-200 dark:border-slate-600" />
                            </h2>
                            {senses.length > 0 && (
                                <>
